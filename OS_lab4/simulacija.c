...
int main() {
   ...
   while (ima > 0) {
      ...
      if (P[0] != NULL &&
          threadNum != 0) {   // ako ima dretvi u redu i ako br dretvi nije nula
         P[0]->p -= kvant;    // umanjenje preostalog vremena dretve za kvant
         if (threadNum > 1) { // ako nije jedina dretva
            for (int i = 0; i < threadNum - 1; i++) {
               if (P[i] != NULL) { // ako dretva postoji u redu
                  temp = P[i];     // zamijeni mjesta sa sljedecom
                  P[i] = P[i + 1];
                  P[i + 1] = temp;
               } else {
                  break;
